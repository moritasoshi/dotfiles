...
moriso.load_config = function()
   ...
   if chadrc_exists then
      ...
      if type(user_config) == "table" then
         conf.mappings = conf.mappings and moriso.prune_key_map(conf.mappings, user_config.mappings, ignore_modes) or {}
         user_config.mappings = user_config.mappings
               and moriso.prune_key_map(user_config.mappings, "rm_disabled", ignore_modes)
            or {}
...
moriso.reduce_key_map = function(key_map, ignore_modes)
   local prune_keys = {}
...
moriso.remove_disabled_mappings = function(key_map)
   local clean_map = {}
...
moriso.prune_key_map = function(key_map, prune_map, ignore_modes)
   ...
   local prune_keys = type(prune_map) == "table" and moriso.reduce_key_map(prune_map, ignore_modes)
      or { n = {}, v = {}, i = {}, t = {} }
   ...
   for ext, modes in pairs(key_map) do
      for mode, mappings in pairs(modes) do
         ...
         key_map[ext][mode] = moriso.remove_disabled_mappings(mappings)
      end
   end
...
moriso.no_WhichKey_map = function()
   local mappings = moriso.load_config().mappings
   local ignore_modes = { "mode_opts" }
...
moriso.remove_default_plugins = function(plugins)
   local removals = moriso.load_config().plugins.remove or {}
...
moriso.merge_plugins = function(default_plugins)
   local user_plugins = moriso.load_config().plugins.user
...
moriso.load_override = function(default_table, plugin_name)
   local user_table = moriso.load_config().plugins.override[plugin_name]
